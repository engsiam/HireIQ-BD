...
export default function AgentDashboard() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-10 space-y-4 md:space-y-6 lg:space-y-12 bg-background min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-8">
        <OverviewCard title="My Properties" value={stats.totalProperties} icon={Building2} change="+12% from last month" index={0} />
        <OverviewCard title="Total Views" value={stats.totalViews} icon={Eye} change="+5% from last month" index={1} />
        <OverviewCard title="Inquiries" value={stats.totalInquiries} icon={MessageSquare} change="+3 new" index={2} />
        <OverviewCard title="Revenue" value={`৳${stats.totalRevenue.toLocaleString()}`} icon={TrendingUp} change="+24% from last month" index={3} />
      </div>
