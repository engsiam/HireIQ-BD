  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="text-white/50 text-sm">Deep insights into your platform performance</p>
      </div>

      {/* Stats Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((stat) => (
          <Card key={stat.title} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl">
        ...
        <Card className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl">
          ...
          <CardContent className="p-4">
            {loading ? (
                  ...
                  return (
                    <div key={index} className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-white/60">{month.month}</span>
                        <span className="text-white/40 text-xs">{month.jobs} jobs / {month.applications} apps</span>
                      </div>
              ...
              <div className="h-48 flex items-center justify-center text-white/40">No data available</div>
            )}
                  ...
                  return (
                    <div key={index} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-white/60 text-xs">{cat.count} jobs</span>
                      </div>
                  ...
                  return (
                    <div key={index} className="p-3 bg-white/5 rounded-lg">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm text-white/80">{item.status}</span>
                      </div>
        ...
        <Card className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl">
          ...
          <CardContent className="p-4">
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                <span className="text-white/70">Avg. Applications per Job</span>
                <span className="text-white font-bold">
                  {stats && stats.totalJobs > 0 ? Math.round(stats.totalApplications / stats.totalJobs) : 0}
                </span>
              </div>
              <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                <span className="text-white/70">Active Jobs Rate</span>
