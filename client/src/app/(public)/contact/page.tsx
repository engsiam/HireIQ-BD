      ...
      <section className="relative py-24 overflow-hidden border-b border-border/50">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10 text-center space-y-6">
          ...
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-muted-foreground max-w-2xl mx-auto font-medium text-lg leading-relaxed">
            Whether you're looking for your dream job or have a business inquiry, our support team is here to assist you.
          </motion.p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-4 space-y-8">
            <div className="space-y-6">
              ...
              {[
                { icon: MapPin, label: 'Head Office', value: 'Gulshan, Dhaka, Bangladesh', desc: 'Visit our office for career consultations.' },
                { icon: Mail, label: 'Email Support', value: 'info@hireiq.com.bd', desc: 'Expect a response within 2 business hours.' },
                { icon: Phone, label: 'Phone', value: '+880 123-456-789', desc: 'Mon-Fri from 9:00 AM to 6:00 PM.' }
              ].map((item, index) => (
            ...
            <div className="p-10 bg-primary rounded-3xl text-white relative overflow-hidden group shadow-2xl shadow-primary/20">
              <Clock className="mb-6" size={40} />
              <h3 className="text-3xl font-black mb-4 leading-none">Need Career Help?</h3>
              <p className="font-bold opacity-80 leading-relaxed text-lg">Our team is available for urgent hiring inquiries and career support.</p>
          ...
          <div className="lg:col-span-8">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-card p-8 md:p-16 rounded-3xl border border-border shadow-2xl relative overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-4">
                  ...
                  <Input id="subject" placeholder="e.g. Job Inquiry" value={formData.subject} onChange={(e) => setFormData({ ...formData, subject: e.target.value })} required className="h-16 bg-secondary/50 border-border rounded-2xl px-6 text-foreground placeholder:text-muted-foreground focus-visible:ring-primary/50 text-lg transition-all" />
                </div>
                <div className="space-y-4">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-12 overflow-hidden rounded-3xl border border-border bg-card h-[400px] relative group">
              ...
              <div className="absolute top-6 left-6 bg-background/80 backdrop-blur-md px-4 py-2 rounded-xl border border-border text-xs font-black uppercase tracking-widest text-foreground">HireIQ HQ - Gulshan</div>
            </motion.div>
          </div>
        </div>
      </section>
